...
const App = () => {
  const [singleSelected, setSingleSelected] = useState<CheckboxButton | null>(
    null,
  );
  ...
  const [
    verticalSelected,
    setVerticalSelected,
  ] = useState<CheckboxButton | null>(null);
  const [
    fastAnimationSelected,
    setFastAnimationSelected,
  ] = useState<CheckboxButton | null>(null);
  const [alwaysSelected, setAlwaysSelected] = useState<CheckboxButton | null>(
    null,
  );
  ...
  const horizontalCheckboxGroupContainer = () => (
    <>
      ...
      {singleSelected && (
        <Text style={styles.selected}>Selected: {singleSelected.id}</Text>
      )}
    </>
  );
  ...
  const alwaysSelectCheckboxGroupContainer = () => (
    <>
      ...
      <Text style={styles.description}>
        Once an option is selected, it can't be deselected. You must select
      ...
      {alwaysSelected && (
        <Text style={styles.selected}>Selected: {alwaysSelected.id}</Text>
      )}
    </>
  );
  ...
  const verticalCheckboxGroupContainer = () => (
    <>
      ...
      {verticalSelected && (
        <Text style={styles.selected}>Selected: {verticalSelected.text}</Text>
      )}
    </>
  );
  ...
  const fastAnimationCheckboxGroupContainer = () => (
    <>
      <Text style={styles.sectionTitle}>Custom Animation Speed (100ms)</Text>
      <View style={styles.checkboxContainer}>
        <BouncyCheckboxGroup
          data={staticData}
          spacing={8}
